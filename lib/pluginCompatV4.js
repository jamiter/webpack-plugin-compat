...
const majorVersion = parseInt(require("tapable/package.json").version.split(".")[0]);

const customHooksMap = new WeakMap();

function canonicalizeName(name) {
	return name.replace(/[- ]([a-zA-Z])/g, str => str.substr(1).toUpperCase());
}

function setHook(obj, name, hook) {
	const hooks = Object.assign({}, customHooksMap.get(obj), {[name]: hook});

	customHooksMap.set(obj, hooks);
}

function getHook(obj, name) {
	const hooks = Object.assign({}, obj.hooks, customHooksMap.get(obj));

	// Special case for 'parser'
	if (name === 'parser' && (hooks.parser instanceof HookMap)) {
		return hooks.parser.for('javascript/auto');
	}
	var hook = hooks[canonicalizeName(name)];
	...
	while (mapParts.length > 1) {
		...
		const hookMap = hooks[canonicalizeName(mapParts.join(' '))];
...
function reg(obj, a1, a2) {
	...
	events.forEach(event => {
		...
		switch(hookType) {
			case "Sync": setHook(obj, name, new SyncHook(args)); break;
			case "SyncBail" : setHook(obj, name, new SyncBailHook(args)); break;
			case "SyncWaterfall" : setHook(obj, name, new SyncWaterfallHook(args)); break;
			case "AsyncParallel" : setHook(obj, name, new AsyncParallelHook(args)); break;
			case "AsyncParallelBail" : setHook(obj, name, new AsyncParallelBailHook(args)); break;
			case "AsyncSeries" : setHook(obj, name, new AsyncSeriesHook(args)); break;
			case "AsyncSeriesWaterfall" : setHook(obj, name, new AsyncSeriesWaterfallHook(args)); break;
			default: {
...
function callHook(hookClass, obj, name, ...args) {
	...
	if (majorVersion <= 1 && !(hook instanceof hookClass) || majorVersion > 1 && hook.constructor.name !== hookClass.name) {
